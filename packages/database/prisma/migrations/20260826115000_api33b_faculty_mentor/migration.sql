-- AlterEnum
ALTER TYPE "GlobalRole" ADD VALUE 'FACULTY_MENTOR';

-- Add Missing UPDATE policy for global admins
CREATE POLICY "Allow global admins to UPDATE users" ON users 
FOR UPDATE USING (current_user_global_role() IN ('PLATFORM_ADMIN', 'FACULTY_ADMIN'));
