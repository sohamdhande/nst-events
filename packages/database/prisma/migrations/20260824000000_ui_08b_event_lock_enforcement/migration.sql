-- PHASE UI-08B: Event Lock Enforcement
-- Lazily enforces event lock rules using database server time.

-- 1. Modify lock_event to respect 24h grace period
...
BEGIN
  ...
  -- ENFORCE: Cannot lock after the 24-hour grace period
  IF now() >= v_event.end_time + interval '24 hours' THEN
...
-- 2. Modify unlock_event to respect 24h grace period
...
BEGIN
  ...
  -- ENFORCE: Cannot unlock after the 24-hour grace period
  IF now() >= v_event.end_time + interval '24 hours' THEN
    RAISE EXCEPTION 'EVENT_LOCKED';
  END IF;
...
-- 3. Modify register_event to respect lazy locking
...
-- 4. Modify mark_attendance to respect lazy locking
...
-- 5. Modify sync_offline_attendance to respect lazy locking
...
BEGIN
  ...
  LOOP
    BEGIN
      ...
      SELECT s.event_id, e.is_locked, s.geofence_radius, e.location_geofence, e.end_time
      INTO v_event_id, v_is_locked, v_geofence_radius, v_location_geofence, v_event_end_time
      FROM attendance_sessions s
      JOIN events e ON s.event_id = e.id
