-- TEST-ATTENDANCE-02: Formalize and Verify Attendance RPC Defect Patches

-- 1. DROP THE OBSOLETE OVERLOAD
-- The old signature sync_offline_attendance(uuid, jsonb) is no longer used by any
-- production code, test, trigger, or deployed caller. The API service uses
-- the single-argument version sync_offline_attendance(jsonb).
DROP FUNCTION IF EXISTS public.sync_offline_attendance(uuid, jsonb);

-- 2. REDEFINE MARK_ATTENDANCE
-- Enforces U0009 for NULLs, U0010 for NaNs/out-of-bounds.
...
-- 3. REDEFINE SYNC_OFFLINE_ATTENDANCE
-- Fixes identity BOLA by using current_user_id() unconditionally.
-- Fixes 24h offline lock by using v_scan_timestamp.
