...
BEGIN
  ...
  IF v_venue_latitude IS NOT NULL AND v_venue_longitude IS NOT NULL THEN
    IF NOT ST_DWithin(ST_SetSRID(ST_MakePoint(v_venue_longitude, v_venue_latitude), 4326)::geography, ST_SetSRID(ST_MakePoint(p_longitude, p_latitude), 4326)::geography, v_geofence_radius + LEAST(p_gps_accuracy, v_max_gps_buffer)) THEN
...
BEGIN
  ...
  LOOP
    BEGIN
      ...
      IF v_venue_latitude IS NOT NULL AND v_venue_longitude IS NOT NULL THEN
        IF NOT ST_DWithin(ST_SetSRID(ST_MakePoint(v_venue_longitude, v_venue_latitude), 4326)::geography, ST_SetSRID(ST_MakePoint(v_lng, v_lat), 4326)::geography, v_geofence_radius + LEAST(v_gps_accuracy, v_max_gps_buffer)) THEN
          RAISE EXCEPTION 'OUTSIDE_GEOFENCE' USING ERRCODE = 'U0007';
        END IF;
      END IF;
...
CREATE OR REPLACE FUNCTION public.mark_attendance_v5(p_session_id uuid, p_totp_token text, p_latitude double precision, p_longitude double precision, p_device_id text, p_device_os text, p_gps_accuracy double precision, p_mock_location_detected boolean, p_app_version text)
 RETURNS attendance_records
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'pg_catalog'
