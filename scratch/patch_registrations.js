...
`export const searchEligibleInvitees = async (userId: string, eventId: string, q: string) => {
  return withUserContext(userId, async (tx) => {
    ...
    const registeredUserIds = existingRegistrations.map((r: any) => r.userId);
    ...
    const pendingInviteeIds = pendingInvitations.map((i: any) => i.inviteeId);
    ...
    // 5. Return top 10
    return eligibleProfiles.slice(0, 10).map((u: any) => ({
      user_id: u.id,
